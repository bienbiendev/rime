...
| 1a       | _this commit_         | **`augment-panel.ts` is gone**; `upload` states `_dashboardLayout` and the dashboard owns its own defaults                                 |
...
> **The ordered backlog lives in `docs/cold-start.md` §4** — nine items, cheapest first, each with
