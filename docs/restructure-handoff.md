...
| 23       | `14fb6d2a`            | **`contentOwnerSlug` is gone**; `persistRelational` resolves the owning slug off `PrototypeHandle.shadow` and `pipeline/persist/*` drops `config` as well             |
...
| 26       | _this commit_         | **upload's path block leaves `insertPrototype`** — `src/lib/adapter-sqlite` imports no individual feature at all now                                                  |
