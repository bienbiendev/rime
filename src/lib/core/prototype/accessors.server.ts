 ...
 *
 * It therefore imports the two accessor types straight from each prototype's `api.server.ts`,
 * and nothing else. It used to live in `registry.server.ts` and read them off each definition's
 * `$InferAccessor` phantom — which was fine until a definition started carrying its own hooks.
 * Then the loop closed: hook → HookContext → locals.rime → accessors → registry → definition →
 * hook, and TypeScript answered `any` for every hook in the repo.
 *
 * The phantom itself is still worth having on the definition (it documents what a kind
