...
import { isLockHeldByOther, isLockStale, lockHolderId } from './lock.js';
...
/**
 * Carry a claim onto the row just made from the one it sits on.
 *
 * An auto-save or a fork continues the same edit on a new row. Made by the holder, the new row
 * is theirs from the first moment: the claim is cleared on `doc` and stamped on `to`, and the
 * client keeps renewing whatever row its form is on. Nothing moves when `doc` is held by somebody
 * else, by nobody, or by a claim that has aged out.
 */
export const moveEditLock = async (args: LockArgs & { to: Partial<GenericDoc> }) => {
  const { doc, to, userId } = args;
  if (lockHolderId(doc) !== userId || isLockStale(doc.currentlyEditedAt)) return false;

  await writeLock(args, { currentlyEditedBy: null, currentlyEditedAt: null });
  await writeLock(
    { ...args, doc: to },
    { currentlyEditedBy: userId, currentlyEditedAt: new Date() }
  );
  return true;
};
