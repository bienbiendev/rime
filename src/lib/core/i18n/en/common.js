export default {
  api_key_dialog_title: 'Your API Key',
  api_key_dialog_text:
    'This API key will only be shown once and cannot be retrieved later. Please copy it and store it securely on your server.',
  auto_save_banner_other: '$1 has a more recent auto-save from $2.',
  auto_save_banner_own: 'You have a more recent auto-save from $1.',
  auto_save_by: 'Auto-save by $1',
  auto_save_discard: 'Discard',
  auto_save_open: 'Open',
  auto_save_paused: 'Auto-save paused: $1',
  auto_save_resume: 'Resume',
  auto_saved_at: 'Auto-saved $1',
  auto_saving: 'Saving…',
  browse: 'browse',
  bulk_upload_dialog_title: 'Bulk upload',
  cancel: 'Cancel',
  close: 'Close',
  confirm: 'Confirm',
  confirmPassword: 'Confirm password',
  create_first: 'Create your first $1',
  create_new: 'Create new $1',
  create_folder: 'New folder',
  create: 'Create',
  created_at: 'Created',
  created_by: 'Created by',
  updated_by: 'Updated by',
  delete: 'Delete $1',
  delete_dialog_title: 'Delete $1',
  delete_dialog_text: 'You are about to delete $1, press delete to confirm the operation.',
  delete_document: 'Delete the document',
  delete_version: 'Delete this version',
  delete_version_dialog_title: 'Delete this version',
  delete_version_dialog_text:
    'You are about to delete this version of the document. The other versions stay.',
  duplicate: 'Duplicate',
  duplicate_success: 'Duplicated successfully',
  doc_deleted: 'Document deleted',
  doc_created: 'Document created',
  doc_updated: 'Document updated',
  draft_infos: 'The document is in draft mode, only panel user can read it',
  draft: 'Draft',
  drop_file: 'Drop your file here',
  drop_files: 'Drop your files here',
  edit_link: 'Edit link',
  forgotPassword: 'Forgot password',
  generatingPreview: 'Generating preview',
  generic_error: 'An error occured $1',
  generic_success: 'Operation successfull',
  import_default_locale: 'Import data from $1',
  last_update: 'Last update',
  leave_confirm_title: 'You have unsaved changes',
  leave_confirm_text: 'Change that you made will be lost',
  live_in_sync: 'Establishing connection with container',
  newPassword: 'New password',
  no_document: 'No $1 found',
  or: 'or',
  passwordResetLinkSent: 'An email has been sent to $1 with instructions to reset the password',
  publish: 'publish',
  published_infos: 'The document is published and available for everyone',
  published: 'Published',
  requestPasswordReset: 'Request a password reset link',
  rename_success: 'Renamed successfully',
  resetPassword: 'Reset password',
  reset_password_success: 'Your password has been reset successfully',
  save_draft: 'Save draft',
  save_new_draft: 'Save in a new draft',
  save: 'Save',
  search_a: 'Search a $1...',
  search: 'Search $1...',
  search_all_placeholder: 'A collection, an area, a document…',
  commands: 'Commands',
  collection: 'Collection',
  search_in: 'Search in $1',
  show_as_list: 'Show as a list',
  show_as_grid: 'Show as a grid',
  show_as_tree: 'Show as a tree',
  mark_as: 'Mark as $1',
  type_a_command: 'Type a command…',
  nothing_found: 'Nothing found',
  go_to: 'Go to',
  document: 'Document',
  collections: 'Collections',
  areas: 'Areas',
  text: 'Text',
  select_all: 'Select all',
  sendPasswordResetLink: 'Send a password reset link',
  signin: 'Sign In',
  someone: 'Someone',
  unsaved_dialog_title: 'Changes unsaved',
  unsaved_dialog_text: 'There are unsaved modifications in the document.',
  version_created: 'New version created',
  version_deleted: 'Version deleted',
  versions_history: 'Versions history',
  view_all: 'View all',
  view_page: 'View page',
  view_site: 'View website',
  welcome: 'Welcome',
  your_auto_save: 'Your auto-save'
};
