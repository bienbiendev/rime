 ...
 * prototype registry is annotated so that no feature's hooks reach `BuildConfig` (rule 1). That
 * makes it the one thing here that can drift from what actually runs — and drift is silent: the
 * config would be typed as if a feature's `configure` had not run, or had run somewhere else.
 *
