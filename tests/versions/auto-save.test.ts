...
test('An auto-save needs the version it starts from', async ({ request }) => {
  ...
  // A RimeError out of an action is answered with `error(status)`, not `fail()`.
  expect(response.status()).toBe(400);
  expect((await response.json()).type).toBe('error');
});

test('The first auto-save is a new row nobody reads by default', async ({ request }) => {
  const headers = await signInSuperAdmin(request);
  ...
  // Branched from the published version: that is what it becomes when saved.
  ...
  // Neither the published read nor the newest-row read sees it, whatever its status says.
  const published = await readNews(request, headers);
  expect(published.attributes.title).toBe('Auto-saved news');
  const newest = await readNews(request, headers, `?${PARAMS.LATEST}=true`);
...
test('An auto-save keeps its status whatever the body says', async ({ request }) => {
  const headers = await signInSuperAdmin(request);
  ...
  expect(row.status).toBe(VERSIONS_STATUS.PUBLISHED);
...
test("Somebody else's auto-save can be written, and is theirs from then on", async ({
  request
}) => {
  const other = await signInOther(request);
  ...
  const response = await panelUpdate(
    ...
    other,
    { [PARAMS.AUTO_SAVE]: 'true', [PARAMS.VERSION_ID]: adminAutoSaveId },
    { 'attributes.title': 'Auto-saved news, taken over' }
  );
  expect((await response.json()).type).toBe('success');

  const row = await readNews(request, other, `?${PARAMS.VERSION_ID}=${adminAutoSaveId}`);
  expect(row.isAutoSave).toBe(true);
  expect(row.attributes.title).toBe('Auto-saved news, taken over');
  expect(row.updatedBy?.id).toBe(otherId);
});

test('Duplicating copies the newest real row, or the row asked for', async ({ request }) => {
  const headers = await signInSuperAdmin(request);
  ...
  const copy = await request
    .get(`${API_BASE_URL}/news/${id}?${PARAMS.LATEST}=true`, { headers })
    .then((r) => r.json());
  expect(copy.doc.attributes.title).toBe('Auto-saved news (copy)');
  expect(copy.doc.isAutoSave).toBe(false);

  // Named by its versionId, an auto-saved row is copied too, into a real document.
  const fromRow = await request.post(
    `${API_BASE_URL}/news/${newsId}/duplicate?${PARAMS.VERSION_ID}=${adminAutoSaveId}`,
    { headers }
  );
  expect(fromRow.status()).toBe(200);
  const rowCopy = await request
    .get(`${API_BASE_URL}/news/${(await fromRow.json()).id}?${PARAMS.LATEST}=true`, { headers })
    .then((r) => r.json());
  expect(rowCopy.doc.attributes.title).toBe('Auto-saved news, taken over (copy)');
  expect(rowCopy.doc.isAutoSave).toBe(false);
});
...
test("A save from the panel promotes the row and retires the saver's other auto-saves", async ({
  ...
  // No status in the body: the row keeps the published one it inherited.
  const response = await panelUpdate(
    request,
    headers,
    { [PARAMS.VERSION_ID]: adminAutoSaveId },
    { 'attributes.title': 'Auto-saved news, saved' }
  );
  expect((await response.json()).type).toBe('success');
  ...
  expect(versions.filter((v) => v.status === VERSIONS_STATUS.PUBLISHED).map((v) => v.id)).toEqual([
    adminAutoSaveId
  ]);
});
